"""Maple external-runtime adapter for RETFound PAPILA glaucoma inference.

Contract:
    predict(input_path: str, output_dir: str, config: dict) -> dict

Identical structure to RETFound_DR_APTOS2019_GradCAM's runner.py (single-image
input, no multi-file manifest needed), adjusted for the 3-class glaucoma head.
"""
...
def _validate_predictions(predictions: Any) -> list[dict[str, Any]]:
    if not isinstance(predictions, list) or len(predictions) != 3:
        raise TypeError("RETFound PAPILA inference must return three class prediction dicts")
    ...
    for expected_index, row in enumerate(predictions):
        ...
        if int(row["pred"]) != expected_index:
            raise ValueError("Prediction class indices must be ordered 0 through 2")
        normalized.append(
            {
                "pred": int(row["pred"]),
...
def predict(input_path: str, output_dir: str, config: dict) -> dict:
    ...
    model_name = str(config.get("model_name") or "RETFound_Glaucoma_PAPILA")
