...
def predict(input_path: str, output_dir: str, config: dict) -> dict:
    ...
    slice_indices = measurements[0]["segmentation_slices"] if measurements else []
    ...
    for image, slice_index in zip(images, slice_indices):
        ...
        raw = buffer.getvalue()
        image_path = output / f"{stem}_lung_segmentation_slice_{slice_index:04d}.png"
    ...
    mask_path = output / f"{stem}_lung_mask.nii.gz"
