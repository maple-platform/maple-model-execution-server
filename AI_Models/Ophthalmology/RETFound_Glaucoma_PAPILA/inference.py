"""Maple inference entrypoint for RETFound PAPILA glaucoma classification.
...
This package mirrors RETFound_DR_APTOS2019_GradCAM's structure exactly (same
RETFound-MAE ViT-L/16 backbone, same preprocessing, same Grad-CAM method) with
a different fine-tuned checkpoint (PAPILA instead of APTOS2019) and a 3-class
head (glaucoma) instead of 5-class (diabetic retinopathy).
"""
...
# Verified against the actual RETFound benchmark PAPILA data split
# (PAPILA.zip from BENCHMARK.md): torchvision.datasets.ImageFolder assigns
# label indices by sorted folder name, and the split folders are named
# "anormal" / "bsuspectglaucoma" / "cglaucoma" specifically to fix this order.
LABELS = {
...
def _safe_torch_load(path: Path) -> Any:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Use the official RETFound PAPILA checkpoint and PyTorch 2.5 or newer."
        ) from exc
...
def _load_model(model_path: str, device: torch.device) -> torch.nn.Module:
    ...
    with _CACHE_LOCK:
        ...
        if real_missing or unexpected:
            raise RuntimeError(
                "Checkpoint is not the expected RETFound-MAE PAPILA 3-class model. "
                f"missing={real_missing}, unexpected={list(unexpected)}"
            )
        if model.head.out_features != len(LABELS):
            raise RuntimeError(f"Expected 3 output classes, got {model.head.out_features}.")
...
def main(input_data, model_path: str):
    ...
    if not model_path:
        raise ValueError("model_path must point to the official fine-tuned PAPILA checkpoint.")
