...
| `"dicom"` | `str` (파일 경로) | DICOM 파일의 절대 경로 |
| `"nifti"` | `str` (파일 경로) | NIfTI 파일(`.nii.gz`)의 절대 경로 |
...
{
  "model_name": "GradCAM++",
  "description": "BME 분류 모델. SI_Joints_Detection의 ROI 결과를 받아 GradCAM++ 히트맵을 반환한다.",
...
```json
{
  ...
  "description":       "모델 설명 — 질환명, 입력 데이터, 출력 형태를 포함",
  "task_type":         "classification | segmentation | bbox detection | ...",
  "disease":           "관련 질환 키워드, 쉼표로 구분",
  "model_type":        "standalone | pipeline",
  "required_data":     ["csv | dicom | nifti"],
...
> `output_image_role`은 `result_type`이 `"image"`인 경우에만 작성합니다. 텍스트 결과 모델은 `null`로 두세요.  
...
| `required_data` | 연구원 | 입력 파일 형식. `"csv"`, `"dicom"`, `"nifti"` 중 해당하는 것 |
| `result_type` | 연구원 | `"text"` (분류/수치 결과) 또는 `"image"` (이미지 결과) |
| `output_image_role` | 연구원 | 결과 이미지의 종류. AI Agent VLM이 이미지를 해석할 때 참고. `result_type: "image"`인 모델만 작성 |
...
| 값 | 설명 | 사용 모델 |
|---|---|---|
| `"bbox_overlay"` | Bounding box가 그려진 원본 영상 오버레이 | SI Joints Detection |
| `"gradcam_overlay"` | GradCAM/GradCAM++ 히트맵 오버레이 | BME Classification |
| `"segmentation_overlay"` | 세그멘테이션 마스크 오버레이 또는 3D 렌더링 | nnUNet_SMWI_Segmentation |
...
```json
{
  ...
  "description":       "파킨슨 낙상 위험도 예측 모델. 임상 Gait 데이터(CSV)를 입력받아 낙상 위험군을 분류한다.",
  "task_type":         "classification",
...
```json
{
  ...
  "description":       "파킨슨병 뇌 심부 구조(RN, SN+STN, Pu) 자동 분할 모델. SMWI NIfTI 영상을 입력받아 3D 렌더링 및 axial slice overlay 이미지를 반환한다.",
...
{
  "model_name":        "YOLOv12",
  "description":       "천장관절(Sacroiliac Joint) 탐지 모델. DICOM MRI 영상을 입력받아 좌우 천장관절 위치를 bounding box로 검출한다.",
...
{
  "model_name":        "GradCAM++",
  "description":       "BME(Bone Marrow Edema) 분류 모델. DICOM MRI를 입력받아 좌우 천장관절의 BME 여부를 분류하고 GradCAM++ 히트맵 이미지를 반환한다.",
  "task_type":         "classification",
  "disease":           "axial spondyloarthritis, bone marrow edema, BME",
  "model_type":        "pipeline",
  "required_data":     ["dicom"],
...
컨테이너가 실행된 상태에서 `POST /run`에 샘플 데이터를 전송해 추론이 정상 동작하는지 확인합니다.

```bash
# 예: SI Joints Detection (Port 9001)
curl -X POST http://localhost:9001/run \
  -H "Content-Type: application/json" \
  -d '{"image_path": "/data/input/sample_01.dcm"}'
```

정상 응답 형태:
```json
{"status": "ok", "result_type": "image", "image_b64": "..."}
```
...
- [ ] `meta.json` — `result_type: "image"` 모델은 `output_image_role` 필드 작성 여부
...
| 진료과 | 모델 폴더명 | 입력 | 결과 | output_image_role |
|---|---|---|---|---|
| Neurology | ParkinsonGait_ML | CSV | 분류 텍스트 | — |
| Neurology | nnUNet_SMWI_Segmentation | NIfTI | 세그멘테이션 이미지 | `segmentation_overlay` |
| Rheumatology | SI Joints Detection | DICOM | Bounding Box 이미지 | `bbox_overlay` |
| Rheumatology | BME Classification | DICOM | GradCAM 이미지 + 분류 확률 | `gradcam_overlay` |
