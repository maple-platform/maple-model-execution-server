...
def setup_task(cfg: Dataclass, **kwargs):
    task = None
...
def register_task(name, dataclass = None):
    ...
    :func:`~fairseq_ecg.tasks.register_task` function decorator.

    For example::

        @register_task('classfication')
        class ClassificationTask(Task):
            (...)
    
    .. note::

        All Tasks must implement the :class:`~fairseq_ecg.tasks.Task`
        interface.
    ...
    def register_task_cls(cls):
        ...
        if not issubclass(cls, Task):
            raise ValueError(
                "Task ({}: {}) must extend Task".format(name, cls.__name__)
            )
        ...
        if dataclass is not None:
            ...
            node = dataclass()
            node._name = name
            cs.store(name = name, group = "task", node = node, provider = "fairseq_")
...
import_tasks(tasks_dir, "fairseq_signals.tasks")
